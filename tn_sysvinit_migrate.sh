...
stage_20_verify_cryptroot() {
    ...
    # Check if LUKS is actually configured on this system NOW
    if [ -f /etc/crypttab ] && grep -qvE '^#|^[[:space:]]*$' /etc/crypttab; then
        info "/etc/crypttab has LUKS entries"
        info "System booted to sysvinit (PID 1 = $(ps -p 1 -o comm=))"
        info "LUKS decryption provably occurred in initramfs"
        return 0
    else
        info "No LUKS configuration in /etc/crypttab"
        return 0
    fi
}
